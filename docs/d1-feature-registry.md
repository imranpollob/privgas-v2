...
| `deploy_log@app` | `factory` | G | AA | counterfactual: B1/B2 and B4-CrossAccount deploy the account in the application op, B3 in the Bootstrap op | account deployment is AA; its placement is caused by the gas mechanism |
...
| `uoe@app` | `factory` | G | AA | counterfactual: B1/B2 and B4-CrossAccount deploy the account in the application op, B3 in the Bootstrap op | account deployment is AA; its placement is caused by the gas mechanism |
...
| `uoe@app` | `nonce` | G | AA | counterfactual: the B3 Spend uses nonce 1 because the Bootstrap consumed nonce 0 (B1/B2 and the B4-CrossAccount Spend: nonce 0) | AA-shaped field whose value is set by the gas mechanism |
...
| `r2_eq_g_account` | R2 | eq | G | G | B3-PrivGas-v1, B4-CrossAccount | `pool_redeem_log.sender`, `pool_deposit_log.sender` | CreditSpent(nullifier, sender) names the same account as the issuance's CreditPool depositor |
| `r2_eq_aa_sender` | R2 | eq | AA+G | AA | B3-PrivGas-v1, B4-CrossAccount | `uoe@app.sender`, `uoe@bootstrap.sender` | Spend UserOperation.sender == Bootstrap UserOperation.sender |
| `r2_eq_t_account` | R2 | eq | T+G | T+G | B3-PrivGas-v1, B4-CrossAccount | `erc20_in_op.sender`, `pool_deposit_log.sender` | the account whose application transfer the Spend made == the issuance's depositor |
| `r2_eq_deploy` | R2 | eq | AA+G | AA | B3-PrivGas-v1, B4-CrossAccount | `uoe@app.sender`, `deploy_log@bootstrap.sender` | Spend sender == account deployed by the issuance operation |
| `r2_rank_gap_g` | R2 | timing | G | G | B3-PrivGas-v1, B4-CrossAccount | `pool_redeem_log.block_number`, `pool_deposit_log.block_number` | |rank of the redemption among redemptions - rank of the issuance among issuances| / (N-1)  (insertion-order / FIFO signal) |
| `r2_rank_gap_aa` | R2 | timing | AA+G | AA | B3-PrivGas-v1, B4-CrossAccount | `uoe@app.block_number`, `uoe@bootstrap.block_number` | same rank gap read from the UserOperationEvent rows |
| `r2_dt_offset_g` | R2 | timing | G | G | B3-PrivGas-v1, B4-CrossAccount | `pool_redeem_log.block_timestamp_utc`, `pool_deposit_log.block_timestamp_utc` | |(t_redeem - t_issue) - (median t_redeem - median t_issue)| / (run span), an unsupervised common-delay window |
| `r2_latest_issuance_g` | R2 | timing | G | G | B3-PrivGas-v1, B4-CrossAccount | `pool_redeem_log.block_number`, `pool_deposit_log.block_number` | candidate is the most recent issuance before the redemption (nearest prior) |
| `r2_intervening_g` | R2 | timing | G | G | B3-PrivGas-v1, B4-CrossAccount | `pool_redeem_log.block_number`, `pool_deposit_log.block_number` | number of issuances between the candidate and the redemption / N (root age proxy) |
| `r2_root_match` | R2 | pm | G | G | B3-PrivGas-v1, B4-CrossAccount | `pool_redeem_log.merkle_root`, `pool_deposit_log.merkle_root` | the redemption proved against the root emitted by the candidate's deposit |
| `r2_bits_agree` | R2 | pm | G | G | B3-PrivGas-v1, B4-CrossAccount | `pool_redeem_log.nullifier`, `pool_deposit_log.commitment` | fraction of equal bits in the low 64 bits of nullifier and commitment (cryptographic negative control) |
| `r2_gas_rank_gap` | R2 | gas | G | G | B3-PrivGas-v1, B4-CrossAccount | `uoe@app.actual_gas_used`, `uoe@bootstrap.actual_gas_used` | |rank of Spend gas used - rank of Bootstrap gas used| / (N-1) |
| `r2_pvg_gap` | R2 | gas | G | G | B3-PrivGas-v1, B4-CrossAccount | `uoe@app.pre_verification_gas`, `uoe@bootstrap.pre_verification_gas` | |z(Spend PVG) - z(Bootstrap PVG)| (within-run z-scores) |
| `r2_cand_gas_z` | R2 | gas | G | G | B3-PrivGas-v1, B4-CrossAccount | `uoe@bootstrap.actual_gas_used` | within-run z-score of the Bootstrap op's gas used (insertion-position dependent) |
| `r2_cand_pvg_z` | R2 | gas | G | G | B3-PrivGas-v1, B4-CrossAccount | `uoe@bootstrap.pre_verification_gas` | within-run z-score of the Bootstrap op's PVG (calldata zero bytes) |
| `r2_delivery_rank_gap_tg` | R2 | timing | T+G | T+G | B3-PrivGas-v1, B4-CrossAccount | `erc20_tx.block_number`, `erc20_tx.subject_account`, `pool_deposit_log.sender`, `erc20_in_op.block_number` | |rank of token delivery to the candidate's depositor - rank of the Spend's application transfer| / (N-1) |
| `r3_dir_vs_action_rank_t` | R3 | timing | T | T | B1, B2-Signature, B2-Allowlist, B3-PrivGas-v1, B4-CrossAccount | `AUX.directory_position`, `erc20_in_op.block_number` | |directory position of the wallet - rank of the application transfer| / (N-1) |
...
| `r3_dir_vs_delivery_rank` | R3 | timing | T | T | B0, B1, B2-Signature, B2-Allowlist, B3-PrivGas-v1, B4-CrossAccount | `AUX.directory_position`, `erc20_tx.block_number`, `erc20_tx.subject_account`, `erc20_in_op.sender` | |directory position - rank of the token delivery to the account| / (N-1) |
| `r3_dir_vs_op_rank_aa` | R3 | timing | AA | AA | B1, B2-Signature, B2-Allowlist, B3-PrivGas-v1, B4-CrossAccount | `AUX.directory_position`, `uoe@app.block_number` | |directory position - rank of the UserOperation| / (N-1) |
...
| `r3_dir_vs_issuance_rank_g` | R3 | timing | G | G | B3-PrivGas-v1 | `AUX.directory_position`, `pool_deposit_log.block_number`, `pool_deposit_log.sender`, `pool_redeem_log.sender` | |directory position - rank of the spender's issuance| / (N-1). B3 only: it is defined through spender == depositor, which does not exist in B4-CrossAccount |
| `r3_addr_hamming_t` | R3 | eq | T | T | B0, B1, B2-Signature, B2-Allowlist, B3-PrivGas-v1, B4-CrossAccount | `AUX.wallet_address`, `erc20_tx.sender`, `erc20_in_op.sender` | normalised Hamming distance between the bits of the account that makes the application transfer and the wallet address |
| `r3_addr_prefix_t` | R3 | eq | T | T | B0, B1, B2-Signature, B2-Allowlist, B3-PrivGas-v1, B4-CrossAccount | `AUX.wallet_address`, `erc20_tx.sender`, `erc20_in_op.sender` | shared leading hex nibbles of that account and the wallet address / 40 |
| `r3_gas_rank_vs_dir_g` | R3 | gas | G | G | B1, B2-Signature, B2-Allowlist, B3-PrivGas-v1, B4-CrossAccount | `AUX.directory_position`, `uoe@app.actual_gas_used` | |rank of the op's gas used - directory position| / (N-1) |
| `r3_pvg_rank_vs_dir_g` | R3 | gas | G | G | B1, B2-Signature, B2-Allowlist, B3-PrivGas-v1, B4-CrossAccount | `AUX.directory_position`, `uoe@app.pre_verification_gas` | |rank of the op's PVG - directory position| / (N-1) |
